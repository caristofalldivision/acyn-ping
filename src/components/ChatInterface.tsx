  ...
  return <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-shrink-0 p-4 border-t border-border bg-card/50 backdrop-blur-sm">
        <div className="max-w-3xl mx-auto flex gap-3">
          <Button onClick={handleMicClick} variant="ghost" size="icon" className={`rounded-xl flex-shrink-0 ${isListening ? 'bg-primary/20 text-primary' : 'hover:bg-secondary'}`}>
            <Mic className="w-5 h-5" />
          </Button>
          <Input value={input} onChange={e => setInput(e.target.value)} onKeyPress={e => e.key === "Enter" && !loading && sendMessage()} placeholder="Type your message..." disabled={loading} className="flex-1 rounded-xl bg-input border-border/50 focus:border-primary/50 h-11 px-4" />
          <Button onClick={sendMessage} disabled={loading || !input.trim()} className="rounded-xl px-4 h-11">
