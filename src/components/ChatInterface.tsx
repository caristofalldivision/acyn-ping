...
const KineticCore = lazy(() => import("./KineticCore").then(m => ({
  default: m.KineticCore
})));
interface Message {
  role: "user" | "assistant";
  content: string;
...
export const ChatInterface = ({
  ...
  const detectDownloadableContent = (content: string): {
    filename: string;
    mimeType: string;
  } | null => {
  ...
  return <div className="flex flex-col h-full overflow-hidden">
      ...
      <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-4 py-6">
        {showEmptyState ? <div className="h-full flex flex-col items-center justify-center max-w-2xl mx-auto">
            ...
            {messages.map((msg, idx) => <div key={idx} className={`flex gap-3 fade-in ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`max-w-[85%] sm:max-w-[80%] rounded-2xl px-4 py-3 ${msg.role === "user" ? "bg-primary text-primary-foreground" : "bg-card border border-border"}`}>
                  {msg.role === "assistant" ? <>
                      ...
                      {detectDownloadableContent(msg.content) && <div className="mt-3 pt-3 border-t border-border">
                          <DownloadButton content={msg.content} filename={detectDownloadableContent(msg.content)!.filename} mimeType={detectDownloadableContent(msg.content)!.mimeType} />
                        </div>}
                    </> : <p className="text-sm sm:text-base">{msg.content}</p>}
                </div>
                {msg.role === "user" && <div className="flex-shrink-0">
            ...
            {loading && <div className="flex gap-3 justify-start fade-in">
        ...
        {showScrollButton && <Button onClick={scrollToBottom} size="icon" className="fixed bottom-28 right-6 rounded-full shadow-lg bg-card border border-border hover:bg-secondary z-10" variant="ghost">
      ...
      <div className="flex-shrink-0 p-4 pb-6">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center gap-3 rounded-2xl px-4 py-3" style={{
          background: "hsl(var(--card) / 0.8)",
          border: "1px solid hsl(var(--border) / 0.5)"
        }}>
            <Sparkles className="w-5 h-5 text-primary flex-shrink-0" />
            <Input value={input} onChange={e => setInput(e.target.value)} onKeyPress={e => e.key === "Enter" && !loading && sendMessage()} placeholder="Ask me anything..." disabled={loading} className="flex-1 border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 h-auto py-0 px-0 text-base placeholder:text-muted-foreground" />
            <Button variant="ghost" size="icon" className="rounded-full flex-shrink-0 hover:bg-secondary h-9 w-9">
              <Paperclip className="w-5 h-5 text-muted-foreground" />
            </Button>
            <Button onClick={() => sendMessage()} disabled={loading || !input.trim()} size="icon" className="rounded-full h-9 w-9 bg-primary hover:bg-primary/90 flex-shrink-0">
